...
## ⚠️注意事項・ユーザー向けガイド 
このゲームはサーバー上でミニゲームとして楽しめるよう設計されていますが、通常のMinecraftプレイとは異なる挙動もあるため、以下の点にご注意ください。

### ❌ゲームの中断・再開について
-ゲーム開始後に途中でログアウトしても、中断処理は行われません。
 - スコア加算処理は継続されます。
 - ただし、再ログイン後にスコアボードが表示されなくなったり、終了メッセージ・テレポートなどの処理が実行されないことがあります。
...
- フィールド上の一部ブロックは鉱石出現によって一時的に置き換えられます。
 ...
 - ブロック破壊に関しては、保護されていますが、完全な衝突防止ではないため周囲に注意してください。
...
**難易度設定の拡張性**
- 難易度別に鉱石の種類、出現率、制限時間を切り替え可能。
- GameDifficultyクラスを拡張することで、新しい難易度の追加もスムーズにできるように。

**テレポートシステムでプレイをスムーズに**
- ゲーム開始時 : プレイヤーを自動で専用エリアに転送
- ゲーム終了時 : 元のワールド、座標に復元(任意)
  - /backコマンドで任意のタイミングで帰還可能
-ブロック復元タイミング前にゲーム開始時の座標にテレポート
　- ゲームが終了すると自動でブロックが復元されるため、ダメージ回避のため

**スコアボードの表示で没入感を演出**
- ActionBarに残り時間、スコアを常時表示し、プレイヤーが進行状況を分かりやすいように。
...
- 特殊イベント or 妨害イベント
- ゲーム中の中断処理、コマンドで即終了の機能

-----

### * おわりに *
このプロジェクトは「ミニゲームとしての楽しさ」と「オブジェクト指向」を両立することを目指して開発しました。
スコアボードやコンボ、難易度設定といった要素に加え、プレイヤーの装備や位置の安全な復元、ブロック復旧処理など、プレイ体験を損なわない工夫を多数取り入れるようにしました。
現時点ではまだ未対応な点もありますが今後のアップデートでより快適に遊べるゲームへと進化させていく予定です。

##  📬ご意見・フィードバック
もし、バグ報告や改善提案がありましたら、ぜひお気軽にお知らせください！
(GiyHub Issue や Pull Request 等)
